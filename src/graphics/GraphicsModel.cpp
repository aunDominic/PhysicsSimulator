...
GraphicsModel::GraphicsModel() {
    // Initialize members so lazy init can detect uninitialized state.
    VAO = VBO = EBO = 0;
...
void GraphicsModel::initBuffers() {
    ...
    // If normals have been provided, initialize normal buffers now
    if (!normalVertices.empty()) {
        initNormalBuffers();
    }
}

void GraphicsModel::ensureInitialized() {
    // If VAO is zero, try to initialize. This is a simple lazy-init guard.
    if (VAO == 0) {
        initBuffers();
    }
}
