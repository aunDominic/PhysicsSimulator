...
BoxModel::BoxModel(glm::vec3 halfExtents) : halfExtents(halfExtents){
    ...
    // Define the 6 cube faces (each face references 4 corner indices)
    faceIndices = {
        {0, 1, 2, 3}, // Front
        {4, 5, 6, 7}, // Back
        {0, 1, 5, 4}, // Bottom
    ...
    createBox();
    // Do not call `initBuffers()` here; it performs GL calls that require
    // a valid OpenGL context. Call `initBuffers()` after context creation.
}
}