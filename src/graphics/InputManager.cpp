...
InputManager::InputManager(){
    InputManager::loadInputConfig("/Users/dominic/CS Projects/Rigid Body Physics /input_config.json");
...
void InputManager::handleInput(float dt){
    if (toReceive == nullptr) {
        spdlog::warn("InputManager::handleInput called but no Inputable set (toReceive is null)");
        return;
    }
...
void InputManager::handleInput(float dt, RigidBody *body){
    if (body == nullptr) {
        spdlog::warn("InputManager::handleInput(dt, RigidBody*) called with null body");
        return;
    }
    if (isKeyPressed("FORWARD")) {
        spdlog::debug("Input FORWARD is detected.");
