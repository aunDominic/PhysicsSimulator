...
void Simulation::initOpenGL() {
    ...
    staticShaderProgram = createShaderProgram(BLUE_VERTEX_SHADER_PATH, BLUE_FRAGMENT_SHADER_PATH);
...
void Simulation::render() {
    ...
    viewLoc = glGetUniformLocation(staticShaderProgram, "view");
    glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view));

    projectionLoc = glGetUniformLocation(staticShaderProgram, "projection");
    glUniformMatrix4fv(projectionLoc, 1, GL_FALSE, glm::value_ptr(projection));

    lightPosLoc = glGetUniformLocation(staticShaderProgram, "lightPos");
    glUniform3f(lightPosLoc, 10.0f, 10.0f, 10.0f);  // Adjust these values
    
    lightColorLoc = glGetUniformLocation(staticShaderProgram, "lightColor");
    glUniform3f(lightColorLoc, 1.0f, 1.0f, 1.0f);  // White light

    ambientStrengthLoc = glGetUniformLocation(staticShaderProgram, "ambientStrength");
    glUniform1f(ambientStrengthLoc, 0.5f);  // Adjust this value as needed    
    ...
    for (const auto& body : system->getStaticBodies()) {
        auto pos = body->getPosition();
        spdlog::debug("Rendering static body at position: ({}, {}, {}) with type:{}", 
                  pos.x, pos.y, pos.z, static_cast<int>(body->geometry->getType()));
        // printf("body: %x, geometry: %x, graphicsModel: %x", body, body->geometry, body->geometry->graphicsModel);
        body->geometry->graphicsModel->render(body->getTransformMatrix());
        body->geometry->graphicsModel->renderNormals(body->getTransformMatrix());
    }
...
void Simulation::setSystem(System *system){
    ...
    for (auto &body: this->system->getStaticBodies()){
        body->geometry->graphicsModel->setShaderProgram(&staticShaderProgram);
    }
